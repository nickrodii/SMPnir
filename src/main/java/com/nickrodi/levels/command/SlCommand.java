...
import com.nickrodi.levels.service.EnchantmentCatalog;
import com.nickrodi.levels.service.FishingCollectionCatalog;
...
public class SlCommand implements TabExecutor {
    private final ProgressionService progressionService;
    private final LevelCurve levelCurve;
    private final StorageService storageService;

    public SlCommand(ProgressionService progressionService, LevelCurve levelCurve, StorageService storageService) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length >= 2 && !isPrimarySub(args[0])) {
        ...
        if (target == null) {
            sender.sendMessage("Usage: /nir <player> <subcommand> ...");
            return true;
        }

        String sub = args[index].toLowerCase(Locale.ROOT);
        switch (sub) {
            case "xp" -> handleXp(sender, target, shiftArgs(args, index));
            case "totalxp" -> handleTotalXp(sender, target);
            case "level" -> handleLevel(sender, target, shiftArgs(args, index));
            case "debug" -> handleDebug(sender, target);
            case "collections" -> handleCollections(sender, target, shiftArgs(args, index));
            case "statistics", "stats" -> handleStatistics(sender, target, shiftArgs(args, index));
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        List<String> primary = List.of("xp", "level", "totalxp", "debug", "collections", "statistics", "stats");
        if (args.length == 1) {
            List<String> options = new ArrayList<>(onlinePlayerNames());
            options.addAll(primary);
            return filter(options, args[0]);
        }

        int index = 0;
        if (args.length >= 1 && !isPrimarySub(args[0])) {
            index = 1;
        }
        ...
        String sub = args[index].toLowerCase(Locale.ROOT);
        int subArgPos = args.length - index;
        if (subArgPos == 1) {
            return filter(primary, args[args.length - 1]);
        }

        if (sub.equals("xp") || sub.equals("level")) {
            if (subArgPos == 2) {
                return filter(List.of("add", "remove", "set"), args[args.length - 1]);
            }
            return List.of();
        }
        ...
        if (sub.equals("collections")) {
            if (subArgPos == 2) {
                return filter(collectionCategories(), args[args.length - 1]);
            }
            String category = args.length > index + 1 ? args[index + 1] : "";
            if (subArgPos == 3) {
                return filter(List.of("grant", "revoke", "clear", "add", "remove"), args[args.length - 1]);
            }
            if (subArgPos == 4) {
                return filter(collectionIds(category), args[args.length - 1]);
            }
            if (subArgPos == 5 && isEnchantCategory(category)) {
                return filter(enchantLevelSuggestions(category, args[index + 2]), args[args.length - 1]);
            }
            return List.of();
        }
        ...
        if (sub.equals("statistics") || sub.equals("stats")) {
            if (subArgPos == 2) {
                return filter(statSections(), args[args.length - 1]);
            }
            if (subArgPos == 3) {
                return filter(List.of("set", "add", "remove"), args[args.length - 1]);
            }
            if (subArgPos == 4) {
                String section = args.length > index + 1 ? args[index + 1] : "";
                return filter(statIds(section), args[args.length - 1]);
            }
        }
    ...
    private void handleXp(CommandSender sender, OfflinePlayer target, String[] args) {
        if (args.length < 3) {
            sender.sendMessage("Usage: /nir <player> xp <add|remove|set> <amount>");
            return;
        }
        ...
        switch (action) {
            case "add" -> {
                if (target.isOnline()) {
                    ...
                    sender.sendMessage("Added " + amount + " XP. Levels gained: " + gained + ".");
                } else {
            ...
            default -> sender.sendMessage("Usage: /nir <player> xp <add|remove|set> <amount>");
        }
    }

    private void handleLevel(CommandSender sender, OfflinePlayer target, String[] args) {
        if (args.length < 3) {
            sender.sendMessage("Usage: /nir <player> level <add|remove|set> <amount>");
            return;
        }
        String action = args[1].toLowerCase(Locale.ROOT);
        int amount;
    ...
    private void handleTotalXp(CommandSender sender, OfflinePlayer target) {
        PlayerData data = loadData(target);
        sender.sendMessage("Total XP: " + data.getTotalXp());
    }

    private void handleDebug(CommandSender sender, OfflinePlayer target) {
        if (!target.isOnline()) {
            sender.sendMessage("Target must be online to toggle debug.");
            return;
        }
    ...
    private void sendHelp(CommandSender sender) {
        ...
        sender.sendMessage("/nir <player> xp <add|remove|set> <amount>");
        sender.sendMessage("/nir <player> level <add|remove|set> <amount>");
        sender.sendMessage("/nir <player> totalxp");
        sender.sendMessage("/nir <player> debug");
        sender.sendMessage("/nir <player> collections <category> <grant|revoke|clear> <id> [level]");
        sender.sendMessage("/nir <player> statistics <section> <set|add|remove> <stat> <amount>");
    }
    ...
    private boolean isPrimarySub(String value) {
        if (value == null) {
            return false;
        }
        String normalized = value.toLowerCase(Locale.ROOT);
        return normalized.equals("xp")
                || normalized.equals("level")
                || normalized.equals("totalxp")
                || normalized.equals("debug")
                || normalized.equals("collections")
